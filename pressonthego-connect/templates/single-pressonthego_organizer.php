...
while ( have_posts() ) :
	...
	$id      = get_the_ID();
	$phone   = get_post_meta( $id, '_OrganizerPhone',   true );
	$email   = get_post_meta( $id, '_OrganizerEmail',   true );
	$website = get_post_meta( $id, '_OrganizerWebsite', true );
...
<article style="max-width:800px;margin:2rem auto;padding:0 1rem;">
	...
	<div style="background:#f8fafc;border:1px solid #e5e7eb;border-radius:8px;padding:1.25rem;margin-bottom:1.75rem;display:flex;flex-wrap:wrap;gap:1rem;">
		...
		<?php if ( $email ) : ?>
		<div>
			<div style="font-size:.6875rem;font-weight:700;text-transform:uppercase;color:#6b7280;margin-bottom:.25rem;letter-spacing:.05em;">Email</div>
			<a href="mailto:<?php echo esc_attr( $email ); ?>" style="color:#185FA5;text-decoration:none;"><?php echo esc_html( $email ); ?></a>
		</div>
		<?php endif; ?>
		...
		<div>
			...
			<a href="<?php echo esc_url( $website ); ?>" target="_blank" rel="noopener noreferrer" style="color:#185FA5;text-decoration:none;">
	...
	<?php // ── Upcoming events ────────────────────────────────────────────── ?>
	<?php if ( $upcoming ) : ?>
	<h2 style="font-size:1.2rem;margin:0 0 1rem;padding-top:1.25rem;border-top:1px solid #e5e7eb;">Upcoming events</h2>
		...
		<a href="<?php echo esc_url( get_permalink( $event ) ); ?>"
		   style="display:flex;gap:1rem;padding:.875rem 1rem;border:1px solid #e5e7eb;border-radius:6px;text-decoration:none;color:inherit;background:#fff;align-items:center;">
			...
			<div>
				...
				<div style="font-size:.8125rem;color:#6b7280;">
					<?php
					$meta = [];
					if ( $e_start && ! $e_all_day ) $meta[] = (new DateTime($e_start))->format('H:i');
					if ( $venue_city )               $meta[] = $venue_city;
